...
test('semantic brand tokens HTML renders and matches screenshot', async ({ page }) => {
  let result = `<h1>Semantic brand tokens</h1><ul>${extractLastKeyValue(token)}</ul>`;
  await page.setViewportSize({ width: 1280, height: 5000 });
  await page.setContent(result);
  await expect(page.locator('h1')).toHaveText('Semantic brand tokens');

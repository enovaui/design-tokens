...
test('core tokens typography HTML renders and matches screenshot', async ({ page }) => {
  let result = `<h1>Core Tokens Typography</h1><ul>${extractLastKeyValue(token)}</ul>`;
  await page.setViewportSize({ width: 1280, height: 4000 });
  await page.setContent(result);
  await expect(page.locator('h1')).toHaveText('Core Tokens Typography');
  await expect(page).toHaveScreenshot('reference-tockens-typography.png', {threshold: 0.2});
});
